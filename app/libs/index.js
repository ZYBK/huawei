...
function getVersion(url2) {
    var a1=url2.lastIndexOf("index_v");
    var a2=url2.lastIndexOf(".js");
    var a3=url2.substring(a1+"index_v".length,a2);
    return a3;
 }
...
function loadApp(url) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        var isDccOk = true;
        var resVersion=getVersion(url);
         console.log("resVersion:" + resVersion);
        function updateDcc() {
            return __awaiter(this, void 0, void 0, function* () {
                cache.setResourceID('appurl', urlpath);
                var curassets = cache.getResourceID('netassetsid');
                console.log('=====load res assetsid:'+urlpath + 'update/assetsid_v'+resVersion+'.txt');
                var assetsidStr = (yield asyncs.downloadSync(urlpath + 'update/assetsid_v'+resVersion+'.txt', false, null));
                 if (!assetsidStr){
                    console.log('=====load res assetsid:'+urlpath + 'update/assetsid.txt');
                    assetsidStr = (yield asyncs.downloadSync(urlpath + 'update/assetsid.txt?rand=' + Math.random() * Date.now(), false, null));
                 }
                ...
                if (!assetsidStr) {
                    ...
                    if (curassets != assetsidStr) {
                        log('need update;');
                        var txtdcc = '';
                        console.log('=====load res filetable:'+urlpath + 'update/filetable_v'+resVersion+'.bin');
                        var bindcc = yield asyncs.downloadSync(urlpath + 'update/filetable_v'+resVersion+'.bin', true, null);
                        if (!bindcc || !(bindcc instanceof ArrayBuffer)) {
                            console.log('=====load res filetable:'+urlpath + 'update/filetable.bin?' + assetsidStr);
                             bindcc =  yield asyncs.downloadSync(urlpath + 'update/filetable.bin?' + assetsidStr, true, null);
                        }
                        ...
                        if(!txtdcc){
                            console.log('=====load res filetable:'+urlpath + 'update/filetable_v'+resVersion+'.txt');
                            txtdcc = (yield asyncs.downloadSync(urlpath + 'update/filetable_v'+resVersion+'.txt', false, null));
                        }

                        if(!txtdcc){
                            console.log('=====load res filetable:'+urlpath + 'update/filetable.txt?' + assetsidStr);
